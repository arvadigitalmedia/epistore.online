<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="GET" action="{{ route('admin.brands.index') }}" class="mb-6">
                        <div class="flex gap-4">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name" class="rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($brand->logo)
                                            <img src="{{ Storage::url($brand->logo) }}" alt="{{ $brand->name }}" class="h-10 w-10 rounded-full object-cover">
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">{{ $brand->products_count ?? 0 }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('admin.brands.edit', $brand) }}" class="text-primary-600 hover:text-primary-900 mr-3">Edit</a>
                                ...
                                @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">No brands found.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <x-modal name="import-brands" focusable>
        <form method="POST" action="{{ route('admin.brands.import') }}" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <p class="mt-1 text-sm text-gray-600">
                {{ __('Please upload an Excel or CSV file containing brand data. The file should have columns: name, description (optional), status (active/inactive).') }}
            </p>

            <div class="mt-6">
                <x-input-label for="file" value="{{ __('File') }}" class="sr-only" />
                <input id="file" name="file" type="file" accept=".xlsx, .xls, .csv" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none" required />
